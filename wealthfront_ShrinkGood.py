# -*- coding: utf-8 -*-
"""
Created on Sun Sep 30 17:11:33 2018
...
if __name__ == '__main__':
    ...
    df2_train, df2_test = train_test_split(df2,test_size = 0.2, random_state = 0)
    #resample the train
    df2_train_bad = df2_train[df2_train['performance']==0]
    df2_train_good = df2_train[df2_train['performance']==1]
    df2_train_good_s1, df2_train_good_s2 = train_test_split(df2_train_good,test_size = 0.03, random_state = 0)
    print(len(df2_train_bad),len(df2_train_good_s2))
    df2_train = pd.concat([df2_train_bad,df2_train_good_s2])
    print(len(df2_train))
    
    X_train = df2_train.iloc[:, 1:22] # features listed in the Task 4    22
    y_train = df2_train.iloc[:, 28]  #performance
    y_train_list = list(y_train)
    print(y_train_list.count(1),y_train_list.count(0))
